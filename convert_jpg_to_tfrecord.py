...
paramenmt_set['dst_folder'] = 'asl_tf' #存放的目录
paramenmt_set['dst_file'] = 'asl' #生成的文件名前缀

paramenmt_set['_NUM_VALIDATION'] = 350 #测试张数
paramenmt_set['_RANDOM_SEED'] = 33  #随机洗乱种子
paramenmt_set['_NUM_SHARDS'] = 1 #tfrecord分割块数
paramenmt_set['channels_number'] = 1 #图片通道数

#填入编码键值，自己随便取
example_name['image'] = 'image/encoded'  #主要是这个(原图)
example_name['label'] = 'image/class/label' #主要是这个(标签)

example_name['format'] = 'image/format'  #这个一般不用理，用不上
example_name['height'] = 'image/height' #这个一般不用理，用不上
example_name['width'] = 'image/width'  #这个一般不用理，用不上
example_name['channel'] = 'image/channel'  #这个一般不用理，用不上
...
def image_to_tfexample(example_name,image_data, image_format, height, width,channel, class_id):
...
class ImageReader(object):
  def __init__(self,channels_number):
    ...
    self._decode_jpeg = tf.image.decode_jpeg(self._decode_jpeg_data, channels=channels_number)
...
def _convert_dataset(example_name,split_name, filenames, class_names_to_ids, paramenmt_set):
  ...
  with tf.Graph().as_default():
    image_reader = ImageReader(channels_number =paramenmt_set['channels_number'] )
      ...
      for shard_id in range(paramenmt_set['_NUM_SHARDS']):
        ...
        with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
          ...
          for i in range(start_ndx, end_ndx):
            ...
            # height, width, channel = image_reader.read_image_dims(sess, image_data)
            ...
            example = image_to_tfexample(
                example_name,img_raw, b'jpg', height, width,channel, class_id)
